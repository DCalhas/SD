 ...
 */
public class IdentificationHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public boolean id(SOAPMessageContext smc) {
		System.out.println("IDENTIFICATIONHANDLER: Handling message.");
		...
		try {
			if (outboundElement.booleanValue()) {				
				...
				Name name = soapEnvelope.createName("wsSenderName", "w", "http://ws.security.komparator.org/");
				SOAPHeaderElement element = soapHeader.addHeaderElement(name);
				if(!Manager.wsName.equals(""))
					element.addTextNode(Manager.wsName);
