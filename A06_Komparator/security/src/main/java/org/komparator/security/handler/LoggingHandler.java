...
public class LoggingHandler implements SOAPHandler<SOAPMessageContext> {
	...
	private void logToSystemOut(SOAPMessageContext smc) {
		...
		try {
			...
			Iterator it = message.getSOAPPart().getEnvelope().getHeader().getAllAttributes();
			while(it.hasNext())
				System.out.println(it.next().toString());
			System.out.println(); // add a newline after message

		} catch (SOAPException se) {
			System.out.print("SOAPException caught in LoggingHandler");
		} catch (IOException e) {
			System.out.println("IOException: failed to write message to System.out");
		} 
	}
